...
public class PreguntaDAO implements IPreguntaDAO  {
	...
	public void crearPregunta(Pregunta objPregunta) {
		...
		con.conectarse();
		try {
			PreparedStatement pstm = (PreparedStatement) con.getConnection().prepareStatement("INSERT INTO pregunta(id_categoria,interrogante) VALUES(?,?)");
	...
	public void modificarPregunta(Pregunta objPregunta) {
		...
		con.conectarse();
		try {
			PreparedStatement pstm = (PreparedStatement) con.getConnection().prepareStatement("UPDATE pregunta SET id_categoria =?,interrogante=? WHERE id_pregunta=?");
			pstm.setInt(1,objPregunta.getPre_id_cate());
			pstm.setString(2,objPregunta.getInterrogante());
	...
	public void eliminarPregunta(Pregunta objPregunta) {
		...
		con.conectarse();
		try {
			PreparedStatement pstm = (PreparedStatement) con.getConnection().prepareStatement("DELETE from pregunta WHERE id_pregunta =?");
	...
	public void listarPregunta() {
		...
		try {
			...
			System.out.println("------------------------------------------------");
			while(rs.next()) {
				System.out.println("\t"+rs.getInt("id_pregunta") + " \t" + rs.getInt("id_categoria") + "\t\t " + rs.getString("interrogante"));
			}
	...
	public int obtenerultimaPregunta() {
		...
		con.conectarse();
		try {
			PreparedStatement pstm = (PreparedStatement) con.getConnection().prepareStatement("SELECT count(*)  as cantidad FROM pregunta");
			ResultSet rs = (ResultSet) pstm.executeQuery();
			rs.next();
	...
	public Pregunta[] listarPreguntaAletoriaCategoria(int cate) {
		...
		con.conectarse();
		try {
			PreparedStatement pstm = (PreparedStatement) con.getConnection().prepareStatement("SELECT id_pregunta,id_categoria,interrogante FROM pregunta WHERE id_categoria = "+ cate);
			ResultSet rs = (ResultSet) pstm.executeQuery();
			...
			for (int i = 0; i < miPregunta.length; i++) {
				...
				miPregunta[i] = new Pregunta(rs.getInt("id_pregunta"),rs.getInt("id_categoria"),rs.getString("interrogante"));	
			}
